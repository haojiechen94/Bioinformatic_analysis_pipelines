...
Differential_analysis<-function(input,metadata,interest_variable,
    ...
    write.table(attr(norm, "norm.coef"),
                gettextf('%s/normalization_coefficiences.txt',
                         outdir),
                sep='\t',quote=F)
    ...
    png(gettextf('%s/%s_vs_%s_MVC_plot.png',outdir,condition1,condition2))
    ...
    png(gettextf('%s/%s_vs_%s_MA_plot.png',outdir,condition1,condition2))
    par(mar=c(5,6,4,2),cex.main=1,font.main=1)
